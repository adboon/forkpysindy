...
class sr3_extreme(BaseOptimizer):
    """
    ...
    See the following references for more details:
        ...
        regularized regression: sr3_extreme." IEEE Access 7 (2018): 1404-1423.
    ...
    >>> from pysindy.optimizers import sr3_extreme
    >>> lorenz = lambda z,t : [10 * (z[1] - z[0]),
    >>>                        z[0] * (28 - z[2]) - z[1],
    >>>                        z[0] * z[1] - 8 / 3 * z[2]]
    >>> t = np.arange(0, 2, .002)
    >>> x = odeint(lorenz, [-8, 8, 27], t)
    >>> opt = sr3_extreme(threshold=0.1, nu=1)
    ...
    def __init__(
        ...
        super(sr3_extreme, self).__init__(
            max_iter=max_iter,
    ...
    def _objective(self, x, y, q, coef_full, coef_sparse, trimming_array=None):
        ...
        if q != 0:
            ...
            print_ind = q
        R2 = (y - np.dot(x, coef_full)) ** self.gamma
    ...
    def _convergence_criterion(self):
        ...
        if self.use_trimming:
            ...
            err_trimming = (
                np.sqrt(np.sum((this_trimming_array - last_trimming_array) ** self.gamma))
    ...
    def _reduce(self, x, y):
        """
        Perform at most ``self.max_iter`` iterations of the sr3_extreme algorithm.
        ...
        if self.verbose:
            row = [
                ...
                "|w-u|^2/v",
                "R(u)",
                "Total Error: |y-Xw|^gamma + |w-u|^2/v + R(u)",
            ]
            print(
                "{: >10} ... {: >10} ... {: >10} ... {: >10}"
                " ... {: >10}".format(*row)
        ...
        for k in range(self.max_iter):
            if self.use_trimming:
                ...
                trimming_grad = 0.5 * np.sum((y - x.dot(coef_full)) ** self.gamma, axis=1)
            ...
            warnings.warn(
                "sr3_extreme._reduce did not converge after {} iterations.".format(
                    self.max_iter
                ),
                ConvergenceWarning,
            )
