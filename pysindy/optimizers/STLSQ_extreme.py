...
class STLSQ_extreme(BaseOptimizer):
    ...
    >>> from pysindy.optimizers import STLSQ_extreme
    >>> lorenz = lambda z,t : [10*(z[1] - z[0]),
    >>>                        z[0]*(28 - z[2]) - z[1],
    >>>                        z[0]*z[1] - 8/3*z[2]]
    >>> t = np.arange(0,2,.002)
    >>> x = odeint(lorenz, [-8,8,27], t)
    >>> opt = STLSQ_extreme(threshold=.1, alpha=.5)
    ...
    def __init__(
        ...
        super(STLSQ_extreme, self).__init__(
            max_iter=max_iter,
    ...
    def _reduce(self, x, y):
        ...
        for k in range(self.max_iter):
            ...
            warnings.warn(
                "STLSQ_extreme._reduce did not converge after {} iterations.".format(
                    self.max_iter
                ),
                ConvergenceWarning,
            )
            try:
                ...
                coef = self.coef_
                warnings.warn(
                    "STLSQ_extreme._reduce has no iterations left to determine coef",
                    ConvergenceWarning,
                )
        self.coef_ = coef
        self.ind_ = ind
